package com.chensoul.foodie.domain.order.service;
...
import com.chensoul.foodie.domain.order.entity.VoucherOrder;
import com.chensoul.foodie.domain.order.mapper.VoucherOrderMapper;
import com.chensoul.foodie.domain.user.entity.User;
import com.chensoul.foodie.infrastructure.lock.RedisLock;
...
public class SeckillService {
	...
	public void doSeckill(final Integer voucherId) throws JsonProcessingException {
		...
		final SeckillVoucher seckillVoucher = objectMapper.readValue((String) redisTemplate.opsForValue().get(key), SeckillVoucher.class);
		...
		final User userInfo = userClient.getCurrentUser().getData();
		final VoucherOrder order = voucherOrderMapper.findUserOrder(userInfo.getId(), seckillVoucher.getVoucherId());
		...
		try {
			...
			if (isLocked) {
				...
				final long count = voucherOrderMapper.save(voucherOrder);
				Assert.isTrue(count != 0, "用户抢购失败");

				final List<String> keys = new ArrayList<>();
				keys.add(key);
				keys.add("amount");
				final Long amount = (Long) redisTemplate.execute(defaultRedisScript, keys);
	...
	public void addSeckillVouchers(final SeckillVoucher seckillVoucher) {
		...
		final Map<String, Object> map = redisTemplate.opsForHash().entries(key);
		...
		redisTemplate.opsForValue().set(key, seckillVoucher);
	}
