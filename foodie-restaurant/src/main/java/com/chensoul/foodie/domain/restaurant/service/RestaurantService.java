package com.chensoul.foodie.domain.restaurant.service;
...
public class RestaurantService extends ServiceImpl<RestaurantMapper, Restaurant> implements IService<Restaurant> {
	...
	public Restaurant findById(final Long restaurantId) {
		...
		final LinkedHashMap restaurantMap = (LinkedHashMap) redisTemplate.opsForHash().entries(key);
		Restaurant restaurant = null;
		if (restaurantMap == null || restaurantMap.isEmpty()) {
			log.info("缓存失效了，查询数据库：{}", restaurantId);
			restaurant = baseMapper.selectById(restaurantId);
			if (restaurant != null) {
				redisTemplate.opsForHash().putAll(key, BeanUtils.beanToMap(restaurant));
			} else {
