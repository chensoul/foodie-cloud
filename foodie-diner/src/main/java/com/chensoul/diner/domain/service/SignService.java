package com.chensoul.diner.domain.service;
...
public class SignService {
	...
	public Map<String, Boolean> getSignInfo(final LocalDateTime date) {
		final String signKey = buildSignKey(SecurityContextHolder.getContext().getAuthentication().getName(), date);
		...
		final BitFieldSubCommands bitFieldSubCommands = BitFieldSubCommands.create()
			...
			.valueAt(0);
		final List<Long> list = redisTemplate.opsForValue().bitField(signKey, bitFieldSubCommands);
	...
	public int doSign(LocalDateTime date) {
		...
		final boolean isSigned = redisTemplate.opsForValue().getBit(signKey, offset);
		Assert.isTrue(!isSigned, "当前日期已完成签到，无需再签");

		redisTemplate.opsForValue().setBit(signKey, offset, true);

		final int count = getContinuousSignCount(username, date);
	...
	private static String buildSignKey(final String username, final LocalDateTime date) {
		return String.format("sign:sign:%s:%s", username, date.format(DateTimeFormatter.ofPattern("yyyyMM")));
	}
