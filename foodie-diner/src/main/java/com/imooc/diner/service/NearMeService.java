...
import com.imooc.diner.vo.NearMeUserVO;
...
public class NearMeService {
	...
	public void updateUserLocation(final Float lon, final Float lat) {
		...
		this.redisTemplate.opsForGeo().add(RedisKeyConstant.USER_LOCATION.getKey(), geoLocation);
	}
	...
	public List<NearMeUserVO> findNearMe(Integer radius, final Float lon, final Float lat) {
		...
		final R<List<User>> users = userClient.findByIds(nearMeUserVOMap.keySet());
