...
public class NearMeService {
	...
	public List<NearMeDinerVO> findNearMe(final String accessToken,
		...
		final SignInDinerInfo signInDinerInfo = this.loadSignInDinerInfo(accessToken);
		// 食客 ID
		final Integer dinerId = signInDinerInfo.getId();
		// 处理半径，默认 1000m
		if (radius == null) {
			radius = 1000;
		}
		...
		// 构建有序 Map
		final Map<Integer, NearMeDinerVO> nearMeDinerVOMap = Maps.newLinkedHashMap();
		// 完善用户头像昵称信息
		geoResult.forEach(result -> {
			final RedisGeoCommands.GeoLocation<Integer> geoLocation = result.getContent();
			// 初始化 Vo 对象
			...
			final String distanceStr = NumberUtil.round(dist, 1).toString() + "m";
		...
		final List<ShortDinerInfo> shortDinerInfos = this.dinersService.findByIds(StrUtil.join(",", dinerIds));
		// 完善昵称头像信息
